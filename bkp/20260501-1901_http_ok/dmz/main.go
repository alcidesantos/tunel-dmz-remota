...
// (HeaderSize, MaxPayload, MessageType consts, ProxyChannelID, SSHTarget, WebProxyChannel, etc.)
...
func handleClient(conn net.Conn) {
	...
	for {
		...
		// 🔹 1. DISPATCH WEB PROXY (Ch:100) -> Encaminha para handleWebProxy
		if channelID == WebProxyChannel && msgType == MsgData {
			...
			continue // ⚠️ Sai aqui. NÃO envia ACK, NÃO vai para o switch
		}

		// 🔹 2. PROXY SSH (Ch:99) -> Encaminha direto
		...
		// 🔹 3. TRATAMENTO PADRÃO (Canais 1, 2, Heartbeat, etc.)
		switch msgType {
		case MsgChannelOpen:
